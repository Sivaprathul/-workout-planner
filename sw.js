const CACHE = 'workout-planner-v10';
...
self.addEventListener('fetch',e=>{
 ...
 if(u.pathname.endsWith('/index.html')||u.pathname.endsWith('/')){
  ...
  e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).then(x=>{const c=x.clone();caches.open(CACHE).then(y=>y.put(e.request,c)).catch(()=>{});return x;})));
 }
});
